...
class RNAAnnotationConfig(BaseConfig):
    def __init__(self, input: Path, output_dir: Path, threads: int, log_file: Path, memory: str, 
                 override_params: dict[str, any] = None, skip_steps: list[str] = None, rnamotif_tool = None,
                 secondary_structure_tool: str = 'RNAfold', ires_tool: str = 'IRESfinder', trna_tool: str = 'tRNAscan-SE',
                 cm_db: str = 'Rfam', custom_cm_db: str = '', output_format: str = 'tsv', motif_db: str = 'jaspar_rna', **kwargs):
        ...
        self.step_params = {
            'RNAfold': {'temperature': 25}, # 37 is unreasonably hot for most enviroments
            'LinearFold': {},  # Fixed parameter name
            'RNAstructure': {'temperature': 25},
            'cmsearch': {'cut_ga': True, 'noali': True},  # Removed E-value as it's incompatible with cut_ga. User can still set it manually..
...
@click.option("-o", "--output-dir", default="./annotate_RNA_output", help="Output directory path")
...
@click.option("-op","--override_params","--override-parameters", default='{}', help='JSON-like string of parameters to override. Example: --override-parameters \'{"RNAfold": {"temperature": 37}, "cmscan": {"E": 1e-5}}\'')
@click.option("--skip-steps", default='', help='Comma-separated list of steps to skip. Example: --skip-steps RNAfold,cmsearch')
@click.option("--secondary-structure-tool", default='LinearFold', type=click.Choice(['RNAfold', 'LinearFold'],case_sensitive=False), help='Tool for secondary structure prediction. LinearFold is faster but less configurable.') # 'SQUARNA', 'RNAstructure',
@click.option("--ires-tool", default='IRESfinder', type=click.Choice(['IRESfinder', 'IRESpy'],case_sensitive=False), help='Tool for IRES identification')
@click.option("--trna-tool", default='tRNAscan-SE', type=click.Choice(['tRNAscan-SE', 'aragorn'],case_sensitive=False), help='Tool for tRNA identification')
@click.option("--rnamotif-tool", default='RNAMotif', type=click.Choice(['RNAMotif', 'aragorn'],case_sensitive=False), help='Tool for RNAmotif identification')
@click.option("--cm-db", default='Rfam', type=click.Choice(['Rfam', 'custom'],case_sensitive=False), help='Database for cmscan')
@click.option("--custom-cm-db", default='', help='Path to a custom cm database in nhmmer/cm format (mandatory to use with --cm-db custom)')
@click.option("--output-format", default='tsv', type=click.Choice(['tsv', 'csv', 'gff3'],case_sensitive=False), help='Output format for the combined results')
@click.option("--motif-db", default='RolyPoly', help='Database to use for RNA motif scanning - RolyPoly, jaspar_core, or a path to a folder containg a pwm/msa files')
def annotate_RNA(input, output_dir, threads, log_file, memory, override_params, skip_steps, secondary_structure_tool, ires_tool, trna_tool, rnamotif_tool, cm_db, custom_cm_db, output_format, motif_db):
...
def predict_secondary_structure_rnafold(config, input_fasta, output_file):
    ...
    with open(output_file, 'w') as out_f:
        for record in parse_fastx_file(str(input_fasta), "fasta"):
            ...
            RNA.file_PS_dot_plot(config.output_dir / f"{record.id}_dotplot.ps", sequence, pl)
...
def predict_secondary_structure_rnastructure(config, input_fasta, output_file):
    ...
    with open(output_file, 'w') as out_f:
        for record in parse_fastx_file(str(input_fasta)):
            ...
            cmd = [
                'Fold',
                '--temperature', str(params.get('temperature', 25)),
                str(temp_seq_file),
                str(ct_file)
            ]
            subprocess.run(cmd)
            ...
            with open(ct_file, 'r') as ct_f:
                ...
                structure = ''.join(['(' if int(line.split()[4]) > int(line.split()[0]) else ')' if int(line.split()[4]) != 0 else '.' for line in lines[1:]])
    ...
    config.logger.info(f"RNAstructure prediction completed. Output written to {output_file}")
...
def predict_secondary_structure_linearfold(config, input_fasta, output_file):
    ...
    with open(output_file, 'w') as out_f, ThreadPoolExecutor(max_workers=config.threads) as executor:
        futures = []
        for record in parse_fastx_file(str(input_fasta)):
    ...
    config.logger.info(f"LinearFold prediction completed. DBN file with mfe predictions written to {output_file}")
...
def search_ribozymes(config):
    ...
    if config.cm_db == 'Rfam':
        cm_db_path = os.path.join(Path(os.environ.get("ROLYPOLY_DATA")),"Rfam", "Rfam.cm")
        tools.append("Rfam")
...
def detect_ires_iresfinder(config, input_fasta, output_file):
    ...
    if not shutil.which('IRESfinder.py'):
        config.logger.error("IRESfinder.py not found in PATH. Please ensure it is installed correctly.")
        return False
    ...
    with open(input_fasta, 'r') as f:
        ...
        with open(output_file, 'w') as out:
            ...
            for seq in sequences:
                ...
                if success:
                    tools.append("IRESpy")
                    ires_data = pl.read_csv(output_file, separator='\t', has_header=False).select([
                    ...
                    out.write(f"{seq.id}\t{ires_data['ires_score']}\t{ires_data['ires_start']}\t{ires_data['ires_end']}\n")
...
def predict_trnas(config):
    if config.trna_tool == 'tRNAscan-SE':
        ...
        config.logger.info(f"Skipping tRNA prediction as {config.trna_tool} is not supported")
...
def predict_trnas_with_tRNAscan(config):
    ...
    if os.path.exists(output_file):
        config.logger.info(f"tRNAscan-SE output file {output_file} already exists. removing it as trnascan-se will get confused")
...
def search_rna_elements(config):
    ...
    try:
        ...
        if motifs:
            config.logger.info(f"Successfully loaded {len(motifs)} motifs from {config.motif_db}")
    ...
    with open(output_file, 'w') as out:
        ...
        for record in parse_fastx_file(str(input_fasta), "fasta"):
            ...
            try:
                ...
                if encoded is None:
                    config.logger.warning(f"Could not encode sequence {record.id} - skipping")
                    continue
                ...
                for motif in motifs:
                    try:
                        ...
                        for pos, score in enumerate(score_list):
                            if score >= min_score:
                                out.write(f"{record.id}\t{motif.name}\t{pos+1}\t{pos+motif.matrix.width}\t{score:.3f}\t+\n")
                    except Exception as e:
                        config.logger.warning(f"Error scanning sequence {record.id} with motif {motif.name}: {e}")
                        continue
            except Exception as e:
...
def process_ribozymes_data(config, ribozymes_file):
    ...
    if not os.path.exists(ribozymes_file) or os.path.getsize(ribozymes_file) == 0:
        config.logger.warning(f"Ribozymes file {ribozymes_file} is empty or does not exist")
        return pl.DataFrame()
    ...
    try:
        ...
        if not has_data:
            config.logger.warning(f"Ribozymes file {ribozymes_file} contains no data (only comments)")
            return pl.DataFrame()
        ...
        columns = ["profile_name", "rfam_id", "sequence_ID", "qaccession", "mdl", "mdl_from", "mdl_to", "seq_from", "seq_to", "strand", "trunc", "pass", "gc", "bias", "score", "evalue", "inc", "ribozyme_description"]
        dtypes =  [pl.Utf8,       pl.Utf8,      pl.Utf8,       pl.Utf8,    pl.Utf8, pl.Int64,  pl.Int64,  pl.Int64,  pl.Int64,  pl.Utf8,  pl.Utf8, pl.Utf8, pl.Utf8, pl.Utf8, pl.Float64, pl.Float64, pl.Utf8, pl.Utf8]
        
        raw_data = read_fwf(ribozymes_file, widths=widths, columns=columns, dtypes=dtypes, comment_prefix='#')
        if raw_data.is_empty():
            config.logger.warning(f"No valid data found in ribozymes file {ribozymes_file}")
            return pl.DataFrame()
...
def read_multiDBN_to_dataframe(MultiDBN_file):
    ...
    with open(MultiDBN_file, 'r') as f_in:
        for record in f_in.read().split('>'):
            ...
            mfe = float(structure_line[1].strip('()')) if len(structure_line) > 1 else -0.1
...
def combine_results(config):
    ...
    try:
        ...
        if 'search_ribozymes' not in config.skip_steps:
            ...
            if os.path.exists(ribozymes_file) and os.path.getsize(ribozymes_file) > 0:
                ...
                if not ribozymes_data.is_empty():
                    ...
                    config.logger.debug(f"Added ribozyme data:\n{ribozymes_data.head()}")
        ...
        if 'predict_trnas' not in config.skip_steps:
            ...
            if trnas_file.is_file():
                if config.trna_tool == 'tRNAscan-SE':
                    ...
                    config.logger.warning("integration of Aragorn output is not supported - the raw output file is available at %s", trnas_file)
        ...
        if 'search_rna_elements' not in config.skip_steps:
            ...
            if rna_elements_file.is_file():
                ...
                if not rna_elements_data.is_empty():
                    ...
                    config.logger.debug(f"Added RNA elements data:\n{rna_elements_data.head()}")
        ...
        if 'predict_secondary_structure' not in config.skip_steps:
            ...
            if structure_file.is_file():
                ...
                if not structure_data.is_empty():
                    ...
                    config.logger.debug(f"Added secondary structure data:\n{structure_data.head()}")
        ...
        if all_results:
            ...
            config.logger.warning("No results to combine - no valid data found in any output files")
...
def convert_record_to_gff3_record(row): # for dict objects expected to be coherced into a gff3
    ...
    sequence_id_columns = ["sequence_id",'query', 'qseqid', 'contig_id', 'contig', 'id', 'name']
