...
from rolypoly.commands.annotation.annotate_RNA import process_RNA_annotations, RNAAnnotationConfig
from rolypoly.commands.annotation.annotate_prot import process_protein_annotations, ProteinAnnotationConfig
...
@click.option("-o", "--output", default="rolypoly_annotation", help="Output file location.")
@click.option("-t", "--threads", default=1, help="Number of threads")
@click.option("-g", "--log-file", default=lambda: Path(os.getcwd()) / "annotate_logfile.txt", help="Path to log file")
@click.option("-M", "--memory", default='6gb', help='Memory in GB. Example: -M 8gb')
@click.option("--override-parameters", default='{}', help='JSON-like string of parameters to override. Example: --override-parameters \'{"RNAfold": {"temperature": 37}, "ORFfinder": {"minimum_length": 150}}\'')
@click.option("--skip-steps", default='', help='Comma-separated list of steps to skip. Example: --skip-steps RNA_annotation,protein_annotation or --skip-steps IRESfinder,RNAMotif or --skip-steps ORFfinder,hmmsearch')
@click.option("--secondary-structure-tool", default='LinearFold', type=click.Choice(["LinearFold", "RNAfold", "SQUARNA", "RNAstructure", "IPknot"]), help='Tool for secondary structure prediction')
@click.option("--ires-tool", default='IRESfinder', type=click.Choice(['IRESfinder', 'IRESpy']), help='Tool for IRES identification')
@click.option("--trna-tool", default='tRNAscan-SE', type=click.Choice(['tRNAscan-SE', 'RNAmotif']), help='Tool for tRNA identification')
@click.option("--rnamotif-tool", default='lightmotif', type=click.Choice(['lightmotif', 'aragorn']), help='Tool for RNA sequence motif identification')
@click.option("--gene-prediction-tool", default='pyrodigal', type=click.Choice(['pyrodigal',"orffinder","six_frames", 'MetaGeneAnnotator']), help='Tool for gene prediction')
...
def annotate(input, output, threads, log_file, memory, override_parameters, skip_steps, secondary_structure_tool, ires_tool, trna_tool, rnamotif_tool, gene_prediction_tool, domain_db, custom_domain_db, min_orf_length, search_tool):
    ...
    if "RNA_annotation" not in config.skip_steps:
        ...
        tools.append("rfam") # TODO: add other needed domain_db to the citation reminder
    else:
