...
@click.option('-agg', "--aggregate", default=False, type=bool, help="aggregate statistics across all sequences")
...
@click.option('--min_length', hidden=True, default=None, type=int, help='minimum sequence length to consider')
@click.option('--max_length', hidden=True, default=None, type=int, help='maximum sequence length to consider')
@click.option('--format', default='tsv', type=click.Choice(case_sensitive=False, choices=['text', 'json', 'tsv']), help='output format')
@click.option('-rt','--rna_tool', default='ViennaRNA', type=click.Choice(case_sensitive=False, choices=['ViennaRNA', 'LinearFold']), help='RNA secondary structure prediction tool')
@click.option('-f', '--fields', default="length,gc_content,n_count,hash,structure,mfe,mfe_per_nt,codon_usage,kmer_freq", help='''
              ...
              ''')
def sequence_stats(input, aggregate, output, threads, memory, log_file, log_level, min_length, max_length, format, rna_tool, fields):
    ...
    field_options = {
        ...
        'mfe_per_nt': {'desc': 'Minimum free energy per nucleotide', 'needs_structure': True},
    ...
    include_structure = any(f in selected_fields for f in ['structure', 'mfe', 'mfe_per_nt'])
    ...
    if include_kmer:
        stats_expr.append(pl.col("sequence").seq.calculate_kmer_frequencies(k=3).alias("kmer_frequencies"))
    
    # Add structure prediction if requested
    if include_structure:
        if rna_tool.lower() == "linearfold":
            stats_expr.append(pl.col("sequence").rna.predict_structure_with_tool("LinearFold").alias("rna_struct"))
        else:
            stats_expr.append(pl.col("sequence").rna.predict_structure_with_tool("ViennaRNA").alias("rna_struct"))
    ...
    if include_codon:
        ...
        if len(codon_df) > 0:
            ...
            codon_data = codon_df.select([
                pl.col("header"),
                pl.col("codons")
            ]).unnest("codons")
            
            df = df.join(codon_data.select(pl.exclude("sequence")), on="header", how="left")
    ...
    if include_kmer:
        try:
            ...
            if format.lower() in ['tsv', 'text']:
                ...
                df = df.join(kmer_df.select(["header", "kmer_freq_str"]), on="header", how="left")
            else:
                # For JSON output, we can keep the struct
                kmer_df = kmer_df.unnest("kmer_frequencies")
                df = df.join(kmer_df.select(pl.exclude("sequence")), on="header", how="left")
    ...
    if len(df) > 0:
        ...
        if include_codon:
            codon_cols = [col for col in df.columns if col not in ['header', 'sequence', 'length', 'gc_content', 'n_count', 'hash', 'mfe', 'mfe_per_nt', 'structure']]
            display_columns.extend(codon_cols)
        
        # Add kmer columns if requested
        if include_kmer:
            kmer_cols = [col for col in df.columns if col.startswith('kmer_') and col != 'kmer_frequencies']
        ...
        if has_nested_cols and format.lower() in ['tsv', 'text']:
            # Find nested columns
            nested_cols = [
                col for col, dtype in zip(display_df.columns, display_df.dtypes)
                if isinstance(dtype, pl.Struct) or isinstance(dtype, pl.List)
            ]
            
            logger.warning(f"Found nested columns which can't be directly written to CSV/TSV: {nested_cols}")
            ...
            logger.info("Converted nested columns to string representation for CSV/TSV output")
                ...
                for col in display_df.columns:
                    if col != 'header':  # Keep header as is
                        try:
                            ...
                            logger.warning(f"Couldn't convert column {col} to string, dropping it")
