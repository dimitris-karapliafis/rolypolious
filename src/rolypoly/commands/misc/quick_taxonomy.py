...
def run_genomad_hmm_search(input_fasta, output_dir, threads, logger):
    ...
    hmm_db = Path(os.environ['ROLYPOLY_DATA']) / "hmmdbs" / "genomad_rna_viral_markers.hmm"
...
def assign_taxonomy(sequence_id, markers, hmm_results, min_score=50):
    ...
    if not rdrp_markers.is_empty():
        ...
        tax['taxonomy'] = best_hit['taxonomy'] if 'taxonomy' in best_hit else 'RNA virus'
...
def quick_taxonomy(input, output, threads,  log_file, log_level, marker_results, format, min_score, summarize):
    """
    ...
    if format == 'json':
        ...
        pl.DataFrame(flat_assignments).write_csv(
            output_path / 'taxonomy_assignments.tsv',
            separator='\t'
        )
        ...
        if summarize:
            ...
            for k, v in summary.items():
                if k != 'taxonomy_distribution':
                    summary_table.add_row(k, f"{v:.1f}" if isinstance(v, float) else str(v))
