...
@click.command()
@click.option('--package-src', default=lambda: str(Path(__file__).parents[4]), help='The source location of the package')
@click.option('--category', help='Command category (subfolder in commands/)')
@click.option('--name', help='Command name')
@click.option('--common-args', default='all', help='Common arguments to include (comma-separated: threads,input,output,memory,logfile,loglevel or "all")')
@click.option('--packages', default='all', help='Packages to import (comma-separated or "all")')
@click.option('--description', help='Command description')
@click.option('--args-json', help='JSON string of custom arguments in format [{"name": "arg1", "type": "str", "required": true}, ...]')
@click.option('--interactive/--no-interactive', default=True, help='Whether to run in interactive mode if CLI args are missing')
def add_command(package_src, category, name, common_args, packages, description, args_json, interactive):
    ...
    if interactive and (not category or not name or not description):
        if not category:
            category = click.prompt('Enter the command category (subfolder of "src/rolypoly/commands/")')
        if not name:
    ...
    if not all([category, name, description]):
        console.print("[red]Error: category, name, and description are required in non-interactive mode[/red]")
        return

    args = []
    if args_json:
        try:
            args = [(arg['name'], arg['type'], arg['required']) for arg in json.loads(args_json)]
...
""")
    ...
    for i, line in enumerate(lines):
        if "lazy_subcommands" in line:
            ...
            for j in range(i+1, len(lines)):
                if f'"{category}"' in lines[j]:
                    ...
                    for k in range(j+1, len(lines)):
                        if '}' in lines[k]:
                            lines.insert(k, f'            "{name}": "rolypoly.commands.{category}.{name}.{name}",\n')
                            break
                    break
            if not category_found:
                ...
                lines.insert(i+4, f'            "{name}": "rolypoly.commands.{category}.{name}.{name}"\n')
                lines.insert(i+5, f'        }}\n')
                lines.insert(i+6, f'    }},\n')
            break
    ...
    click.echo(f"Command {name} created successfully in {command_file} and added to {main_script}.")
