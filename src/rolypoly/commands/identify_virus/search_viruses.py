...
@click.option("-o", "--output", default=lambda: f"{os.getcwd()}_RP_mapping", help="output file location - set suffix to .tab, .sam or html [default: .tab]")
@click.option("--keep-tmp", is_flag=True, default=False, help="Keep temporary files")
@click.option("--db", type=click.Choice(["RVMT", "NCBI_Ribovirus", "all", "other"]), default="all", help="""Select the database to search against.""")
@click.option("--db-path", default="", help="Path to the user-supplied source (required if --db is 'other'). Either a fasta or a path to formatted MMseqs2 virus database")
@click.option("-g", "--log-file", default=lambda: f"{os.getcwd()}/search_viruses_logfile.txt", help="Abs path to logfile")
@click.option("-i", "--input", required=True, help="Input path to nucl fasta file OR preformatted mmseqs db")
def virus_mapping(threads, memory,  output, keep_tmp,  db, db_path, log_file, input):
    ...
    log_start_info(logger,{"input":input,"output":output,"db":db,"db_path":db_path,"threads":threads,"memory":memory,"keep_tmp":keep_tmp,"log_file":log_file})
    ...
    if(pt(input).suffix in [".faa",".fasta",".fas",".fa",".fna",".fastq",".fq",".gz",".fastq.gz",".fq.gz",".fasta.gz",".fa.gz",".fas.gz",".faa.gz"]):
        logger.info(f"Converting input to mmseqs DB")
        tmp=pt(tmpdir) / "pl_sv_contig_db"
        os.makedirs(tmp, exist_ok=True)
        mmseqs_createdb_cmd = (
            f"mmseqs createdb {str(input)} {tmp}/mmdb "
            f" --dbtype 2"
        )
    ...
    if db == "all":
        ...
        if not db_path:
            console.print("[bold red]Error:[/bold red] Please provide a path to the user-supplied database with --db-path")
            return
        if(pt(db_path).suffix in [".faa",".fasta",".fas",".fa",".fna"]):
            ...
            os.makedirs(tmp, exist_ok=True)
            mmseqs_createdb_cmd = (
                f"mmseqs createdb {db_path} {tmp}/cmmdb "
                f" --dbtype 2"
            )
            subprocess.run(mmseqs_createdb_cmd, shell=True, check=True)
            db_path = tmp / "cmmdb"  # Ensure the path is updated correctly after creation
