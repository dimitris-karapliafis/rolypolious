...
class TaxonomyTree:
    ...
    def __init__(self, nodes_df, data_availability_df=None, priority_columns=None):
        """
    ...
    def _build_tree(self):
        ...
        self.leaf_nodes = {
            tax_id for tax_id in self.parent_map.keys()
            if tax_id not in self.children_map or len(self.children_map[tax_id]) == 0
        }
        print(f"Built tree with {len(self.parent_map)} nodes, {len(self.leaf_nodes)} leaf nodes")
            ...
            if current in self.leaf_nodes:
                ...
                if current in self.children_map:
                    ...
                    if aggregation_method == 'any':
                        ...
                        should_have_data = with_data >= 1  # At least one descendant
                    elif aggregation_method == 'majority':
                        should_have_data = total > 0 and (with_data / total) > 0.5
                    ...
                    if should_have_data and current not in self.data_available:
                        ...
                        self.metadata_map[current]['descendant_count_with_data'] = with_data
                        self.metadata_map[current]['descendant_count_total'] = total
    ...
    def find_nearest_leaf_with_data(self, tax_id, max_distance=10, priority_override=None):
        """
        ...
        priority_cols = priority_override if priority_override is not None else self.priority_columns
        ...
        return {
            'leaves': prioritized_leaves,
            'self_is_leaf_with_data': False
        }
    
    def _find_leaves_with_data_in_subtree(self, root_id, max_depth, priority_cols):
        """
    ...
    def find_nearest_with_data(self, tax_id, max_distance=10, priority_override=None):
        """
        ...
        """
        cache_key = (tax_id, max_distance, tuple(priority_override or self.priority_columns))
        ...
        nearest_ancestor = self._find_nearest_ancestor_with_data(tax_id, max_distance, priority_cols)
        
        # BFS to find nearest descendants with data  
        nearest_descendants = self._find_nearest_descendants_with_data(tax_id, max_distance, priority_cols)
    ...
    def find_nearest_with_data_batch(self, tax_ids, max_distance=10, priority_override=None, 
                                     leaves_only=False, return_stats=False):
        """
        ...
        for tax_id in tax_ids:
            result = self.find_nearest_with_data(tax_id, max_distance, priority_override)
            ...
            if return_stats:
                if result['self_has_data']:
                    ...
                    if result['ancestor']:
                        ...
                        stats['distance_distribution'][result['ancestor']['distance']] += 1
                        has_match = True
                    if result['descendants']:
                        stats['descendant_found'] += 1
                        min_desc_dist = min(d['distance'] for d in result['descendants'])
    ...
    def find_nearest_leaf_with_data_batch(self, tax_ids, max_distance=10, priority_override=None,
                                          leaves_only=False, return_stats=False):
        """
        ...
        for tax_id in tax_ids:
            result = self.find_nearest_leaf_with_data(tax_id, max_distance, priority_override)
            results[tax_id] = result
            ...
            if return_stats:
                if result['self_is_leaf_with_data']:
                    ...
                    min_distance = min(leaf['distance'] for leaf in result['leaves'])
    ...
    def find_nearest_with_data_unified(self, tax_id, max_rank='genus',
        ...
        if tax_id in self.data_available:
            result = {
                ...
                'ancestor': self._create_node_info(tax_id, 0),
                'leaves': [self._create_node_info(tax_id, 0)] if tax_id in self.leaf_nodes else [],
                'self_has_data': True,
    ...
    def get_tree_stats(self):
        ...
        stats = {
            ...
            'internal_nodes_with_data': len(self.data_available - self.leaf_nodes),
            'coverage_percent_all': (len(self.data_available) / len(self.parent_map) * 100) if self.parent_map else 0,
            'coverage_percent_leaves': (len(self.leaf_nodes & self.data_available) / len(self.leaf_nodes) * 100) if self.leaf_nodes else 0,
    ...
    def print_stats(self):
        ...
        print(f"  Leaf nodes only:      {stats['coverage_percent_leaves']:.2f}%")
        ...
        print(f"\nTop ranks by count:")
        sorted_ranks = sorted(stats['rank_distribution'].items(), 
        ...
        for rank, count in sorted_ranks:
            ...
            print(f"  {rank:20s}: {count:8,} total ({with_data:6,} with data, {pct:5.1f}%)")
